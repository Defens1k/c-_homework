...
bool push_char(dynamic_string * array,const char symbol) { 
    if (array->current_lenght < array->capacity) {
        ...
        return push_char(array, symbol); // when we resize, we can add symbol 
    }
    return true;
}
////////////////////////////////////////////////////////////////////////////////
...
bool push_string(strings_dynamic_array * array, dynamic_string * ptr) {
    if (array->current_lenght < array->capacity) {
        ...
        array->data = new_data;
        array->capacity *= 2;
        return push_string(array, ptr);  // when we resize a array we can add ptr to array
    }
    return true;
}
////////////////////////////////////////////////////////////////////
...
strings_dynamic_array * read_strings() {
    ...
    while((c = getchar()) != EOF) {
        if (push_char(new_string, c) == false) {
        ...
        if (c == '\n') {        //  end of string, add in array
            if (push_string(new_array, new_string) == false) {
                delete_strings_dynamic_array(new_array);
                delete_dynamic_string(new_string);
                return NULL;
            }
...
dynamic_string * remove_spases_from_string(const char * old_string) {
    ...
    while (old_string[i] != '\0') {
        if (old_string[i] != ' ' || prev != ' ') {   // add only one space in new string
                       if (push_char(new_string, old_string[i]) == false) {
                delete_dynamic_string(new_string);
                return NULL;
            }
        }
